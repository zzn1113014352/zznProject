 ...
 * 
 */
public class DBManager_ {
	private static final Logger logger = Logger.getLogger(DBManager_.class);
	...
	private void init() {
		...
		conn = DBConnectionManager_.getConnection();
		try {
	...
	public static void main(String arg[]) throws SQLException, IllegalArgumentException, IntrospectionException, ClassNotFoundException, InstantiationException, IllegalAccessException, InvocationTargetException {
		DBManager_ db = new DBManager_();
		db.getBeanFromTable("zc", "com.dhcc.modal.system");
		db.close();
	}
}
